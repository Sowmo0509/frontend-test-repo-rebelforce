...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    if (
      emailDomain &&
      restrictedDomains.some((domain) => emailDomain.includes(domain))
    ) {
      console.warn(
        `Login attempt from potentially restricted domain: ${emailDomain}`,
      );
    }
    ...
    if (!rolePermissions.canLogin) {
      throw new UnauthorizedException(
        'Access denied. Your account role does not have login permissions. Please contact your system administrator.',
      );
    }
  ...
  private getRolePermissions(role: string): {
    ...
    const permissions: Record<
      string,
      { canLogin: boolean; canUpload: boolean; canApprove: boolean }
    > = {
